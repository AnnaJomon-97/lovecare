    ...
    */
    ...
    if( count($form_errors) >0 ){
        ...
        if( $_FILES['fl_image']['name'] != "" ) {
            ...
            $upload->config['upload_path'] = 'uploads/categories/';
        ...
        if ( $id > 0 ){
            $sql = "UPDATE `item_categories` SET `name`= '".$name."',`description`='".$description."',`image`='".$image."' WHERE id =".$id;
            if ( $db->execute($sql) ){
            ...
            $sql = "INSERT INTO `item_categories`( `name`, `description`, `image`) VALUES ('".$name."','".$description."','".$image."')";
            if ( $db->execute($sql) ){
        ...
        redirect('admin/item-categories.php');
    }
}
/*
...
$edit_rec = $db->get_row('item_categories',array('id' => $edit));
if ( $edit_rec ){
    $id = $edit_rec->id;
    $name = $edit_rec->name;
...
if ( $del > 0 ) {
    $del_row = $db->get_row('item_categories',array('id' => $del));
    if ($del_row){
        $image = $del_row->image;
        if (file_exists(  base_path($image) )){
            unlink( base_path($image) );
    ...
    $del_rec = $db->delete('item_categories',array('id' => $del));
    if ( $del_rec ){
...
if ($process != 'FORM'){
    ...
    $rec_list = $db->get('item_categories', $whrere, $order_by);
}
...
<div id="page-wrapper">
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Item Categories </h1>
        </div>
        <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->
      ...
      <div class="col-lg-12">
        <div class="panel panel-default">
          <div class="panel-heading">
            <p><?= $id >0 ? 'Edit ' : 'Add ' ?> Item Category</p>
          </div>
          <div class="panel-body">
            <div class="row">
             
              <div class="col-lg-6 col-lg-offset-2">
                <form class="form-horizontal" method="post" action="<?= base_url('admin/item-categories.php') ?>" enctype="multipart/form-data" >
                  ...
                  <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                      ...
                      <a class="btn btn-default" href="<?= base_url('admin/item-categories.php') ?>">Cancel</a>
                    </div>
                  </div>
                </form>
              </div>
...
<?php }else{ ?>
    <div  class="row" >
      <div class="col-lg-12 " style="margin-bottom: 10px">
        <a href="<?= base_url('admin/item-categories.php?option=form') ?>" class =" btn btn-info pull-right"><i class="fa fa-plus "></i> Add Item Category </a>
      </div>
    </div>
    <div class="row">
                  ...
                  <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                      ...
                      <?php foreach ($rec_list as $key => $row) { ?>
                        <tr class="odd gradeX">
                          ...
                          <td><a href="<?= base_url('admin/item-categories.php?edit='.$row['id'] ) ?>" class="btn btn-sm btn-default"> <i class="fa fa-edit"></i> Edit </a></td>
                          <td><a onclick="if (confirm('Delete record ?')) { window.location.href='<?= base_url('admin/item-categories.php?del='.$row['id'] ) ?>';}" class="btn btn-sm btn-default"> <i class="fa fa-trash"></i> Delete </a></td>
                        </tr>
                      <?php } ?>
                        </tbody>
                    </table>
