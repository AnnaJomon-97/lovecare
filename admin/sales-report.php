...
if (isset ($_POST['btnReport'])){
  ...
  $year = (int)$input->post('year');
   $q = "SELECT o.`id` AS order_id,o.`date` AS order_date, itm.`name` AS item_name,oitm.`price`,oitm.`quantity`  FROM order_items oitm 
LEFT JOIN orders o ON oitm.`order_id` = o.`id`
LEFT JOIN items itm ON oitm.`item_id` = itm.`id`
LEFT JOIN deliveries d ON d.`order_id` = o.`id`
WHERE d.`status`= '2' AND  YEAR( o.`date` ) ='". $year ."' AND MONTH( o.`date` ) ='".$month. "'";
...
<div id="page-wrapper">
    ...
    <div class="row" style="padding: 20px 0px">
        ...
        <form class="form-horizontal" action="<?= base_url('admin/sales-report.php') ?> " method="post">
        <?php 
          ...
          <?php  
          $q = "SELECT YEAR(MIN(date)) AS start_year FROM orders WHERE 1 ";
...
if (isset($_POST['btnReport'])){ ?>
        ...
        <h2>Sales report <?=  $month_name."-".$year ?></h2>
                <?php 
                if(count( $rec_list) == 0){
