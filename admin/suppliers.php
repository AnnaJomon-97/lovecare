...
if( isset( $_POST['btnSave'] ) ){
    ...
    */
    ...
    if( count($form_errors) >0 ){
        ...
        if ( $id > 0 ){
            $sql = "UPDATE `suppliers` SET `name`= '".$name."',`address`='".$address."',`email`='".$email."',`phone`='".$phone."' WHERE id =".$id;
            if ( $db->execute($sql) ){
            ...
            $sql = "INSERT INTO `suppliers`( `name`, `address`, `email`, `phone`) VALUES ('".$name."','".$address."','".$email."','".$phone."')";
            if ( $db->execute($sql) ){
...
$edit_rec = $db->get_row('suppliers',array('id' => $edit));
if ( $edit_rec ){
    $id = $edit_rec->id;
    $name = $edit_rec->name;
...
if ( $del > 0 ) {
    $del_rec = $db->delete('suppliers',array('id' => $del));
    if ( $del_rec ){
...
if ($process != 'FORM'){
    ...
    $rec_list = $db->get('suppliers', $whrere, $order_by);
}
/*
      ...
      <div class="col-lg-12">
        <div class="panel panel-default">
          <div class="panel-heading">
             <p><?= $id >0 ? 'Edit ' : 'Add ' ?> Supplier </p>
          </div>
          <div class="panel-body">
            <div class="row">
             
              <div class="col-lg-6 col-lg-offset-2">
                <form class="form-horizontal" method="post" action="<?= base_url('admin/suppliers.php') ?>">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-10">
                      <input type="phsadone" class="form-control" name="phone" value="<?= $phone ?>" >
                      <?= form_error('phone') ?>
                    </div>
                  </div>
                  <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                      ...
                      <a class="btn btn-default" href="<?= base_url('admin/suppliers.php') ?>">Cancel</a>
                    </div>
                  </div>
                </form>
              </div>
...
<?php }else{ ?>
    <div  class="row" >
      <div class="col-lg-12 " style="margin-bottom: 10px">
        <a href="<?= base_url('admin/suppliers.php?option=form') ?>" class =" btn btn-info pull-right"><i class="fa fa-plus "></i> Add Supplier</a>
      </div>
    </div>
    <div class="row">
                  ...
                  <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                      ...
                      <?php foreach ($rec_list as $key => $row) { ?>
                        <tr class="odd gradeX">
                          ...
                          <td><?= $row['email']  ?></td>
                          <td><?= $row['phone']  ?></td>
                          <td><a href="<?= base_url('admin/suppliers.php?edit='.$row['id'] ) ?>" class="btn btn-sm btn-default"> <i class="fa fa-edit"></i> Edit </a></td>
                          <td><a onclick="if (confirm('Delete record ?')) { window.location.href='<?= base_url('admin/suppliers.php?del='.$row['id'] ) ?>';}" class="btn btn-sm btn-default"> <i class="fa fa-trash"></i> Delete </a></td>
                        </tr>
                      <?php } ?>
                        </tbody>
                    </table>
